...
// The Schedule service may be a separate microservice. Fall back to the shared
// backend URL if a dedicated one is not configured.
const BACKEND_URL =
  process.env.BACKEND_SCHEDULES_API_URL ||
...
if (process.env.NODE_ENV !== "production") {
  try {
    console.debug(
      "[schedules-proxy] USING_BACKEND_URL:",
      BACKEND_URL || "(none - set BACKEND_SCHEDULES_API_URL or BACKEND_API_URL)"
    )
  } catch {
...
/**
 * Proxies a JSON request from the browser to the backend Schedule service.
 ...
 * schedule-service calls are routed through these Next.js route handlers.
 ...
 */
export async function proxyToSchedules(
  request: NextRequest,
  backendPath: string
  ...
  try {
    ...
    // 409 conflicts (room/teacher busy) are passed through with their body
    // so the UI can surface the conflict message and AI suggestions.
